...
class LaplaceConditional(EntropyModel):
    def __init__(
        self,
    ...
    def _likelihood(
        self, inputs: Tensor, scales: Tensor, means: Optional[Tensor] = True
    ) -> Tensor:
        half = float(1e-4)
        ...
        # else:
        #     values = inputs
        laplace = torch.distributions.laplace.Laplace(means, scales)
        # scales = self.lower_bound_scale(scales)

        values = torch.abs(inputs)
        upper = laplace.cdf(values + half)
        lower = laplace.cdf(values - half)
